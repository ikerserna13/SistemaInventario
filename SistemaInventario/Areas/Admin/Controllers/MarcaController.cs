...
namespace SistemaInventario.Areas.Admin.Controllers
{
    ...
    public class MarcaController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(Marca marca)
        {
            if (ModelState.IsValid)
            {
                if (marca.Id == 0)
                {
                    ...
                    TempData[DS.Existosa] = "Marca creada existosamente";
                }
                else
                {
                    _unidadTrabajo.Marca.Actualizar(marca);
                    TempData[DS.Existosa] = "Marca actualizada existosamente";
                }
        ...
        {
            var storeDb = await _unidadTrabajo.Marca.Obtener(id);
            if (storeDb == null)
            {
                return Json(new { success = false, message = "Error al borrar Marca" });
            }
            _unidadTrabajo.Marca.Eliminar(storeDb);
            await _unidadTrabajo.Guardar();
